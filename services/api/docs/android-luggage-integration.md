...
7. “继续创作”携带作品项目 ID，恢复最近文字层；点击“保存草图/脚本”会创建带幂等键的服务端不可变版本，并在首版保存后从“草图/脚本”推进到“制作”。
8. 创作台“上传草图”使用系统文件选择器，只接受 JPEG/PNG/WebP 和最多 20 MB；通过上传、隔离检查后才允许开始。“带入秘籍”只展示习得以上条目，并把秘籍 ID 写入意图与工法版本。
9. 作品制作页右上角增加 48dp 流程入口。制作阶段可准备教练建议，但必须先显示“将读取、将产生、数据去向”，用户点“确认并获取”后才执行；取消会保存拒绝记录，不会产生建议。
10. 测试阶段可记录场景、通过/需修改/受阻、观察和问题；开放问题逐条填写整改说明。只有当前版本最近一次复测通过且开放问题为零，“进入作品说明”才可点击，服务端还会再次校验。
11. 作品说明阶段接入三个真实表单：作品说明与隐私自查、学习卡、来源与人机分工。秘籍选择可为空；AI 辅助和外部素材按条件展开必填项。
12. “只保存给自己”不发起提交；家长、教师/班级、知行流分别选择明确去向，并经过二次确认后提交审核。提交成功后显示检查状态，资料在该版本上锁定。
13. 创作档案展示封卷说明、学习复盘、下次改进、三项隐私确认、发布去向和逐条来源记录。封卷后保存新版本会回到制作阶段并要求重新测试。
...
- 创作教练：`POST|GET /v1/creation-projects/{id}/tool-calls`，确认或拒绝使用 `POST /v1/creation-tool-calls/{id}/decision`。
- 作品测试：`POST|GET /v1/creation-projects/{id}/test-records`，整改使用 `POST /v1/creation-test-issues/{id}/resolve`。
- 封卷说明：`GET|PUT /v1/creation-versions/{id}/seal-check`。
- 学习卡与来源谱：`GET|PUT /v1/creation-versions/{id}/learning-card`、`GET|PUT /v1/creation-versions/{id}/provenance-manifest`。
- 提交检查：`POST /v1/creation-projects/{id}/submissions`，必须带 `Idempotency-Key`；仅保存给自己时不要调用。
...
- 工具确认页必须完整显示输入、影响与数据去向；未确认、拒绝或过期的调用不能返回建议。
- 新保存版本后，旧版本的通过记录不能解锁“进入作品说明”。
- 未完成三项隐私确认、学习说明、来源/人机分工，或 AI/外部来源字段不完整时，“确认提交”不可用，服务端也必须拒绝。
- 未选择秘籍的自由创作可以完成学习卡并提交；不能把秘籍数量当成发布条件。
- 提交按钮必须先展示准确去向、审核前不会公开、三份资料会锁定，并由用户再次确认。
- 页面不显示公开总榜、粉丝数、人气排名或连续签到惩罚。
- Android 默认 API 地址与 Docker 对外端口统一；当前 Docker 是 `8010`，模拟器应配置
  `http://10.0.2.2:8010/`。
- Compose 已把模拟器使用的 `MINIO_PUBLIC_ENDPOINT` 默认设为 `10.0.2.2:19000`；
  真机联调时需要用宿主机局域网地址覆盖它，APK 不应收到 `127.0.0.1` 或容器内部的 `minio:9000`。
