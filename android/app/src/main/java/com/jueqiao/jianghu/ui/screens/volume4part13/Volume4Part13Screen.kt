 ...
 *   - 图2 Y=378+231=609,在书框 Y=88-872 范围内安全(余量 263dp)
 *   - 代码后续真机上调过:图2 Y=318→378(见行内注释)
 *
 * 资源来源:
 *   - 背景:设计稿 image 129.png(复用第一卷 img_volume1_bg.png 资源)
 *   - 书框:设计稿 Group 256.png(复用第一卷-2 img_volume1part2_group_256.png 资源)
 *   - 图1:设计稿 image 377.png(已复制为 res/drawable-nodpi/img_volume4part13_image_377.png)
 *   - 图2:设计稿 image 378.png(已复制为 res/drawable-nodpi/img_volume4part13_image_378.png)
 */
@Composable
fun Volume4Part13Screen(
    onBack: () -> Unit = {},
    onOpenVolume4Part14: () -> Unit = {},
) {
    ...
    Box(
        ...
        Box(
            ...
            // 标题"路边则重算"(字号 24,bold,黑色,X=110, Y=67, W=192, H=32)— 与 Vol-4-12 同款,点击跳第四卷-14。
            Text(
                text = "路边则重算",
                color = Color.Black,
                fontSize = 24.sp,
                ...
                modifier = Modifier
                    ...
                    .clickable(onClick = onOpenVolume4Part14),
            )
            ...
            // 图2(image 378.png,X=18, Y=378, W=355, H=231)— 在书框之上、中部。
            // Y=378+231=609,在书框 Y=88-872 范围内安全(余量 263dp)。
            Box(
                modifier = Modifier
                    .align(Alignment.TopStart)
