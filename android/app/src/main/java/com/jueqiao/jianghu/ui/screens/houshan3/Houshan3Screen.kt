...
fun Houshan3Screen(
    ...
    // §21n 用户指令"删掉后山3页面最底部的云" → **本页删掉 #6 ACI57**
    //   (后山1/后山2 保留 → 三页从此刻起不再完全一致,§17 的"三页效果统一"在此被用户指令覆盖)
    // 当前五个周期(上→下):21.4 / 11 / 8 / 10 / 9 s,互不整除
    ...
    Box(
        ...
        // 原本第 6 个(最底部 ACI57)按"最底部必须有一个动画素材"加入,§21n 又按用户指令删除;
        //   后山3 的最底部改由 §21f 的老云 56(y=755)承担,该要求仍成立。
        // 与后山1/后山2 原本是**完全同一套** y 序列、间隔(69/62/66/60/61)、×0.75 尺寸;
        ...
        // 绘制顺序 = 背景 → 雾层 → 这 5 个 → 内容层(旧云/熊猫/标签),不遮挡任何可点区域。
        // §21c/§21d 横向模式分配 —— **按"观感可见性"分配**(与后山1/后山2 一致):
        //   三朵 ACI 的 PNG 是 250/255/237 的近白/纯白,浅底上几乎不可见;
        //   两条 FocusCloudBand 是冷青 #A9C3C0,浅底上对比明显 → 把**看得见的**放进单向组。
        //   #1 ACI58 摆动(21.4s) · #2 FCB左下 **单向→右**(11s) · #3 ACI60 **单向→右**(8s)
        //   #4 FCB中下 **单向←左**(10s) · #5 ACI62 摆动(9s)
