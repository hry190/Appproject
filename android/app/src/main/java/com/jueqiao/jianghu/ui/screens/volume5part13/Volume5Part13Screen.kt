 ...
 *   - 图1(image 402.png,X=18, Y=135, **W=355, H=306**)— 上部(H=311→306 用户 2026-09-12 真机调整)
 *   - 图2(image 403.png,X=18, Y=478, **W=355, H=268**)— 中下部(H=321→268 用户 2026-09-12 真机调整;采用自然高度 355/1.321)
 *
 * 坐标说明:
 ...
 *   - 图2 Y=478+268=746,在书框 Y=88-872 范围内(余量 126dp;H=321→268 用户真机调整)
 *
 * 资源来源:
 *   - 背景:设计稿 image 129.png(复用第一卷 img_volume1_bg.png 资源)
 *   - 书框:设计稿 Group 255.png(复用第一卷 img_volume1_group_255.png 资源)
 *   - 图1:设计稿 image 402.png(已复制为 res/drawable-nodpi/img_volume5part13_image_402.png)
 *   - 图2:设计稿 image 403.png(已复制为 res/drawable-nodpi/img_volume5part13_image_403.png)
 */
@Composable
fun Volume5Part13Screen(
    ...
    Box(
        ...
        Box(
            ...
            // 图1(image 402.png,X=18, Y=135, W=355, H=306)— 在书框之上、上部(H=311→306 用户真机调整)。
            Box(
                modifier = Modifier
                    .align(Alignment.TopStart)
            ...
            // 图2(image 403.png,X=18, Y=478, W=355, H=268)— 在书框之上、中下部(H=321→268 用户真机调整)。
            // Y=478+268=746,在书框 Y=88-872 范围内(余量 126dp)。
            // ⚠️ 该图原图近正方形(比率 1.321),用户已采纳 H=268(自然高度 355/1.321),现在 0.5% 几乎完美。
            Box(
                modifier = Modifier
                    .align(Alignment.TopStart)
