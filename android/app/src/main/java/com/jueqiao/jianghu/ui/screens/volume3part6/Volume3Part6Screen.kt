 ...
 *   - 图2(image 331.png,X=18, Y=471, W=346, H=322)— 中部(真机调整后版式)
 *
 * 坐标说明:
 ...
 *   - 代码后续真机上调过:图2 Y=351→471, W=446→346, H=422→322(见行内注释)
 *
 * 资源来源:
 *   - 背景:设计稿 image 129.png(复用第一卷 img_volume1_bg.png 资源)
 *   - 书框:设计稿 Group 255.png(复用第一卷 img_volume1_group_255.png 资源)
 *   - 图1:设计稿 image 330.png(已复制为 res/drawable-nodpi/img_volume3part6_image_330.png)
 *   - 图2:设计稿 image 331.png(已复制为 res/drawable-nodpi/img_volume3part6_image_331.png)
 */
@Composable
fun Volume3Part6Screen(
    onBack: () -> Unit = {},
    onOpenVolume3Part7: () -> Unit = {},
) {
    ...
    Box(
        ...
        Box(
            ...
            // 标题"远近藏在数中"(字号 24,bold,黑色,X=110, Y=67, W=192, H=32)— 新标题系列,点击跳第三卷-7。
            Text(
                text = "远近藏在数中",
                color = Color.Black,
                fontSize = 24.sp,
                ...
                modifier = Modifier
                    ...
                    .clickable(onClick = onOpenVolume3Part7),
            )
            ...
            // 图2(image 331.png,X=18, Y=471, W=346, H=322)— 在书框之上、中部。
            // Y=471+322=793,在书框 Y=88-872 范围内安全。
            Box(
                modifier = Modifier
                    .align(Alignment.TopStart)
                    .offset(x = 18.dp, y = 471.dp)
                    .size(width = 346.dp, height = 343.dp),
            ) {
                Image(
