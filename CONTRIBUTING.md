...
> 📌 **`D:\图\` 这个设计稿目录已于 2026-09-20 删除**(用户决定)。它曾经有 410 个文件 / 364.3 MB,
> **完整去向记录**(含体积与 sha256)在 [docs/DESIGN-SOURCES.md](docs/DESIGN-SOURCES.md) §3 ——
> "这张图当初有没有原图、原图多大"都能查。以后素材来源改由用户随时提供,流程不变:

1. 复制进项目并改名成资源命名:`cp "<设计稿目录>\<file>.png" android/app/src/main/res/drawable-nodpi/img_<page>_<node>.png`
2. 在屏幕代码里 `painterResource(R.drawable.img_<page>_<node>)`
3. 注释写「**设计稿 <file>.png → img_<page>_<node>**」—— **留文件名和资源名,不要写盘符路径**。
   理由:盘符路径是**临时坐标**(换机器就失效、目录一删就变成假话),
   而**文件名**是你跟用户沟通时的稳定标识、**资源名**是仓库里能直接找到的东西。
   > 📌 2026-09-20 已按此清理 582 处历史注释(153 个文件)。
4. 顺手在 [docs/DESIGN-SOURCES.md](docs/DESIGN-SOURCES.md) 补一行(设计稿名 → 资源名)——
   那份清单是"**来源目录删了仍可查**"的来源表,注释里不再有路径,靠它兜底。

**大背景图特别提醒**:设计稿动辄 4-5 MB 的 PNG,搬进项目会显著拖大 APK。**搬之前问一下要不要先压缩**(用户那次把 4.9 MB 压到 2.16 MB,效果立竿见影)。
...
- ❌ 不要修改"复制来源"KDoc,除非改的是自己写的(例外:§9.3 的**机械清理**——删盘符路径、补资源名,
  2026-09-20 全仓做过一次,只动这两类内容、不改叙述)
