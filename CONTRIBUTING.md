...
设计稿源文件在本地设计稿目录(`D:\图\`,**不在 git,且计划可能被删除**)下。导入流程:
...
3. 注释写「**设计稿 <file>.png → img_<page>_<node>**」—— **留文件名和资源名,不要写盘符路径**。
   ...
   > 📌 2026-09-20 已按此清理 582 处历史注释(153 个文件),来源对照表见 [docs/DESIGN-SOURCES.md](docs/DESIGN-SOURCES.md)。
4. 顺手在 [docs/DESIGN-SOURCES.md](docs/DESIGN-SOURCES.md) 补一行(设计稿名 → 资源名)——
   那份清单是"**设计稿目录删除后仍可查**"的来源表,注释里不再有路径,靠它兜底。

**大背景图特别提醒**:设计稿目录下动辄 4-5 MB 的 PNG,搬进项目会显著拖大 APK。**搬之前问一下要不要先压缩**(用户那次把 4.9 MB 压到 2.16 MB,效果立竿见影)。
