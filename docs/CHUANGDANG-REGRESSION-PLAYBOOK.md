...
python scripts\verify_fx.py                              # 出手(进攻答对,默认)
python scripts\verify_fx.py --takehit                    # 1a 防御答错 → 怪物扑中 + 熊猫受击
python scripts\verify_fx.py --block                      # 1a 防御答对 → 怪物被挡回 + 熊猫被顶退
python scripts\verify_fx.py --jab                        # 1b 进攻答错 → 怪物扑击起手(只有怪物动)
python scripts\verify_fx.py --windup                     # 1c 防御题出现 → 怪物扑击起手(只有怪物动)
python scripts\verify_fx.py --jab --windup --takehit     # 可连跑(每个模式之间会自动重启 app)
```
...
| # | 判据 | 怎么量 | 通过线 |
|---|---|---|---|
| ① | 动作确实发生 | 出手:相对"回位后那一帧"的**变化区域 bbox** 右缘有没有被顶出去;其余:熊猫**模板匹配 x** + **两侧墨迹重心 x** | 出手 熊猫右缘 **+35px**;格挡 熊猫 **+10px → −4px**、怪物 **−33px → +7.7px**(符号翻转=被挡回);反击起手/防御题起手 怪物 **−31.4 / −31.8px**、熊猫 **±0px**;受击 熊猫 **−28px**、怪物 **−32.5px** |
| ② | 回位无残留 | **同一屏**证据:末尾几帧彼此一致(噪声底)+ 熊猫匹配误差回到静息基准 | 噪声底 熊猫x **0 px** / 怪物重心 **0.00 px**;误差回到 **1438** |
| ③ | **不破坏静态渲染** | 同一个界面**静止**两帧互比 | 熊猫窗口与怪物窗口都应 ≈ 0 px |
| ④ | 动画会结束 | 轨迹要**走完一个脉冲**并停在静息值上 | 稀疏 4 帧不够(看不到曲线),用 `TRACE_N=20` |

> 怪物只有**墨迹重心**这一把尺子(矢量线稿、没有现成模板),它被窗口里的背景**摊薄**:
> 扑击 −12dp≈−33px 读出来正好是 −31~−33px(窗口收得紧,摊薄很小),但**回弹**那半程与
> 整屏淡入重叠,读数被压低(+6dp≈+16.5px 只读到 +7.7px)。**判据用"符号与形状",不用绝对幅度。**
...
3. **基准帧取"尾帧",不要取第 1 帧。** 第 1 帧很可能已经落在动作中间(实测:1b 那一跑第 1 帧
   正好在扑击峰值上 → 被读成"怪物往右跑了 +23px,而且再也不回来")。
   尾帧是"动作早已结束且互相一致"的那一帧,拿它当零点。
4. **跨屏的数字不是读数。** "答题前那一帧"与"结算后那一帧"往往**不是同一屏**(面板高度/内容都变),
   两者相减会把"换屏"算进去 —— 实测假报过一次"怪物没回位 +17.75px"。
   判据② 要拿**同屏**证据:尾帧噪声底 + 匹配误差回基准。
5. **动手之前先确认前置条件成立。** ×20 时换屏过渡也被拉长到几秒,
   "刚拿到题干就点选项"**点不进去**;而工具原先"找不到结算按钮就去找题干"仍能拿到题干
   (还是刚才那道进攻题)→ 于是安静地量了 20 帧"没发生的那一幕"。
   现在:先 `wait_steady()`(连续两次 dump 一致)再点,且 `reach_defense()` 返回 **(ok, nodes, prompt)**
   三件套并校验"新题干 ≠ 刚才那道进攻题"。
