...
> ⚠️ **但"溢出屏幕被裁掉一部分"本身是设计意图,不是缺陷**(2026-09-16 用户确认):
> 有些图版**故意**铺到屏幕外 / 被画框裁掉一截(背景出血、局部出画),这是刻意的视觉效果。
> **排查时按"还能不能看到"分档,不要按越界量分档:**
>
> | 现象 | 判定 |
> |---|---|
> | `y ≥ 屏高`(元素**顶边**已在屏幕下方) | **真 bug** — 整块永远不可见,要修 |
> | `y + h > 可用底边`(873 − 导航栏 16 = **857**) | **通常就是设计意图** — 只是下面一截被裁,不用管 |
>
> 依据:2026-09-16 全 App 扫描出 **158 处"底边越界",其中 157 处是设计意图,只有 1 处是真 bug**
> (Volume3Part7 图2 的 `y=891`,被一个加 Vol-7 页的无关提交误改,见 `SESSION-LOG-2026-09-16.md` §21o)。
> **"普遍存在"本身就是它属于风格、而非错误的证据** —— 别再把这类元素当缺陷清。
